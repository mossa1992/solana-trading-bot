...
def buy_momentum(candidate: dict[str, Any]) -> bool:
    """Require buy dominance plus non-negative price confirmation when available."""
    buys = int(candidate.get("buys_h1") or 0)
    sells = int(candidate.get("sells_h1") or 0)
    if buys <= sells:
        return False
    # Some unit/manual candidates omit price-change fields; keep those compatible.
    # Live DEX candidates include them, so avoid entering a buy-dominant token
    # whose short-term price action is already rolling over.
    for key in ("change_m5_pct", "change_h1_pct"):
        if key in candidate and candidate.get(key) is not None and finite_number(candidate.get(key)) < 0:
            return False
    return True
...
def runtime_cfg() -> dict[str, Any]:
    ...
    cfg = {
        ...
        "signal_threshold": env_float("SIGNAL_THRESHOLD", 72),
