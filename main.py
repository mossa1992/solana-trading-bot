#!/usr/bin/env python3
"""
Solana Meme-Token Signal & Paper Trading Bot
Single-file production version for free cloud hosting.
"""
...
# Default Target ID
DEFAULT_CHAT_ID = "8050745160"

LOG = logging.getLogger("solana-trading-bot")
SCAN_LOCK = threading.Lock()
STOP_EVENT = threading.Event()

# ---------------------------------------------------------
# Health Check Server (Keeps Free Hosting Services Alive)
# ---------------------------------------------------------
...
def ai_assess(candidate: dict[str, Any], cfg: dict[str, Any]) -> dict[str, Any]:
    fallback = {
        ...
        "risks": ["تقلب عالي جداً", "إمكانية تغيير السيولة", *candidate.get("gate_reasons", [])],
    ...
    try:
        ...
        system = "أنت محلل مخاطر لتوكنات سولانا. اخرج JSON حصراً باللغة العربية."
        payload = {
            "model": os.getenv("AI_MODEL", "gpt-4o-mini"),
            "messages": [{"role": "system", "content": system}, {"role": "user", "content": json.dumps(candidate, ensure_ascii=False)}],
            "response_format": {"type": "json_schema", "json_schema": {"name": "signal", "strict": True, "schema": schema}},
        }
        result = post_json(os.getenv("OPENAI_API_BASE").rstrip("/") + "/chat/completions", payload, timeout=45, headers={"Authorization": f"Bearer {os.getenv('OPENAI_API_KEY')}"})
...
def format_signal(candidate: dict[str, Any], assessment: dict[str, Any]) -> str:
    ...
    title = "🎯 إشارة محاكاة تداول" if action == "SIGNAL" else "👁️ مراقبة"
    risks = assessment.get("risks") or []
    risk_text = "؛ ".join(str(x) for x in risks[:5]) or "لا توجد ملاحظات حرجية"
    return (
        ...
        f"التغير: 5د {candidate['change_m5_pct']:.2f}% | 1س {candidate['change_h1_pct']:.2f}%\n"
        f"التدفقات 1س: شراء {candidate['buys_h1']} / بيع {candidate['sells_h1']}\n"
        f"LP مقفلة: {candidate['lp_locked_pct']:.2f}%\n\n"
        f"الخلاصة: {assessment.get('thesis', '')}\n"
        f"المخاطر: {risk_text}\n"
        f"مستويات المحاكاة: دخول {fmt_usd(levels['entry'])} | وقف {fmt_usd(levels['stop'])} | TP1 {fmt_usd(levels['tp1'])} | TP2 {fmt_usd(levels['tp2'])}\n"
...
def paper_open(state: dict[str, Any], candidate: dict[str, Any], cfg: dict[str, Any], now: str) -> dict[str, Any] | None:
    ...
    event = {"type": "open", "mint": mint, "symbol": candidate.get("symbol"), "price": price, "execution_price": exec_price, "quantity": quantity, "notional": notional, "fee": fee, "time": now}
    state.setdefault("trades", []).append(event)
    paper_ledger(event)
    return event
...
def paper_process_results(results: list[dict[str, Any]], signal_threshold: float, min_liquidity: float) -> list[dict[str, Any]]:
    ...
    for mint, pos in list((state.get("positions") or {}).items()):
        ...
        if price <= finite_number(pos.get("stop_price")):
            ...
            if event:
                ...
                if pos:
                    pos["tp1_done"] = True
                    pos["stop_price"] = max(finite_number(pos.get("stop_price")), finite_number(pos.get("entry_price")))
    ...
    for candidate in results:
        ...
        if candidate.get("mint") not in closed_this_cycle and candidate.get("hard_gate_passed") and assessment.get("action") == "SIGNAL" and finite_number(candidate.get("raw_score")) >= signal_threshold:
...
def format_paper_event(event: dict[str, Any]) -> str:
    ...
    if event.get("type") == "open":
        return (
            "<b>📈 محاكاة — فتح صفقة جديدة</b>\n"
            f"<b>${symbol}</b> | القيمة: <b>{fmt_usd(event.get('notional'))}</b>\n"
            f"سعر السوق: {fmt_usd(event.get('price'))} | التنفيذ: {fmt_usd(event.get('execution_price'))}\n"
...
def paper_status() -> str:
    ...
    lines = [
        "<b>📊 تقرير محاكي التداول الورقي (Paper Trading)</b>",
        ...
        f"الرسوم: {fmt_usd(state.get('fees_paid'))} | صفقات مغلقة: {len(trades)} | نسبة النجاح: {winrate:.1f}%",
    ]
    ...
    for pos in (state.get("positions") or {}).values():
        ...
        lines.append(
            ...
            f"القيمة المحاكية: {fmt_usd(net_value)}\n"
            ...
            f"وقف: {fmt_usd(pos.get('stop_price'))} | TP1: {fmt_usd(pos.get('tp1_price'))} | TP2: {fmt_usd(pos.get('tp2_price'))}"
        )
    return "\n".join(lines)
    ...
    for _, profile, pair, _ in ranked[: cfg["max_candidates"]]:
        try:
            ...
            assessment = ai_assess(candidate, cfg) if candidate["hard_gate_passed"] else {
                ...
                "invalidation": "N/A", "entry_plan": "لا دخول", "exit_plan": "لا صفقة", "rationale": "خطر حتمي"
            }
...
def guarded_scan_once(*, send: bool = False, dry_run: bool = False) -> list[dict[str, Any]]:
    if not SCAN_LOCK.acquire(blocking=False):
        raise RuntimeError("فحص جارٍ بالفعل")
    try:
...
def handle_command(update: dict[str, Any]) -> None:
    ...
    chat_id = str((message.get("chat") or {}).get("id") or os.getenv("TELEGRAM_CHAT_ID", DEFAULT_CHAT_ID))
    if command in {"/start", "/help"}:
        reply = (
            "🤖 **أوامر التحكم بالبوت والمحاكي:**\n\n"
            "/paper_status — عرض حالة وتقارير المحاكي الحية\n"
            "/scan — تشغيل فحص فوري وتقييم الفرص\n"
            "/status — عرض إعدادات وحالة البوت\n"
            "/pause أو /resume — إيقاف/استئناف الفحص\n"
            "/paper_reset — إعادة تصفير رصيد المحاكاة إلى 200 USDC"
        )
        ...
        state = load_state()
        cfg = runtime_cfg()
        reply = f"حالة البوت: {'مفعّل' if state.get('enabled') else 'متوقف'}\nآخر فحص: {state.get('last_scan')}\nالسيولة الأدنى: ${cfg['min_liquidity']}"
    elif command == "/paper_reset":
        save_paper_state(paper_default_state())
        if PAPER_LEDGER_FILE.exists():
            PAPER_LEDGER_FILE.unlink()
        reply = "تمت إعادة ضبط حساب المحاكاة إلى 200 USDC افتراضية."
    elif command == "/scan":
        send_telegram("⏳ جارٍ الفحص وتحديث المحاكي...", {}, parse_mode=None)
        try:
            guarded_scan_once(send=False)
            reply = paper_status()
        except Exception as e:
            reply = f"خطأ أثناء الفحص: {e}"
    else:
        reply = "أمر غير معروف. استخدم /help"
    final_payload = {"chat_id": chat_id, "text": reply, "parse_mode": "HTML" if command == "/paper_status" else None}
    post_json(f"{TELEGRAM_BASE}/bot{os.getenv('TELEGRAM_BOT_TOKEN')}/sendMessage", final_payload)
...
def telegram_poll_loop() -> None:
    ...
    while not STOP_EVENT.is_set():
        try:
            data = get_json(f"{TELEGRAM_BASE}/bot{token}/getUpdates?offset={offset}&timeout=20")
...
def main() -> int:
    ...
    LOG.info("Bot started successfully. Targeting Chat ID: %s", os.getenv("TELEGRAM_CHAT_ID", DEFAULT_CHAT_ID))
